...
function healthy_living_theme_color( $color ) {
	$cmsmasters_option = healthy_living_get_global_options();

	$colors = array(
		'color' => $cmsmasters_option['healthy-living_default_color'],
		'link' => $cmsmasters_option['healthy-living_default_link'],
		'hover' => $cmsmasters_option['healthy-living_default_hover'],
		'heading' => $cmsmasters_option['healthy-living_default_heading'],
		'bg' => $cmsmasters_option['healthy-living_default_bg'],
		'alternate' => $cmsmasters_option['healthy-living_default_alternate'],
		'border' => $cmsmasters_option['healthy-living_default_border'],
	);
...
function healthy_living_hex_color( $color ) {
	$old_color = healthy_living_theme_color( $color );
...
function healthy_living_tribe_update_default_options() {
	...
	$new_global_elements = array(
		'event_title_color' => healthy_living_hex_color( 'heading' ),
		'event_date_time_color' => healthy_living_hex_color( 'color' ),
		'link_color' => healthy_living_hex_color( 'link' ),
		'background_color_choice' => 'transparent',
		'background_color' => 'transparent',
		'accent_color' => healthy_living_hex_color( 'link' ),
		'map_pin' => '',
	);
	...
	$new_single_event = array(
		...
		'post_title_color' => healthy_living_hex_color( 'heading' ),
	...
	$new_tec_events_bar = array(
		'events_bar_text_color' => healthy_living_hex_color( 'heading' ),
		'find_events_button_text_color' => healthy_living_hex_color( 'bg' ),
		'events_bar_icon_color_choice' => 'default',
		'events_bar_icon_color' => healthy_living_hex_color( 'heading' ),
		'find_events_button_color_choice' => 'custom',
		'find_events_button_color' => healthy_living_hex_color( 'link' ),
		'events_bar_background_color_choice' => 'default',
		'events_bar_background_color' => healthy_living_hex_color( 'bg' ),
		'view_selector_background_color_choice' => 'default',
		'view_selector_background_color' => healthy_living_hex_color( 'bg' ),
		'events_bar_border_color_choice' => 'default',
		'events_bar_border_color' => healthy_living_hex_color( 'border' ),
	);
	...
	$new_month_view = array(
		'days_of_week_color' => healthy_living_hex_color( 'heading' ),
		'date_marker_color' => healthy_living_hex_color( 'heading' ),
		'multiday_event_bar_color_choice' => 'default',
		'multiday_event_bar_color' => '#334aff',
		'grid_lines_color' => healthy_living_hex_color( 'border' ),
		'grid_hover_color' => healthy_living_hex_color( 'border' ),
		'grid_background_color_choice' => 'transparent',
		'grid_background_color' => healthy_living_hex_color( 'color' ),
...
add_action( 'init', 'healthy_living_tribe_update_default_options' );


/* The Events Calendar and related plugins: Add your own location for template file loading. */
function healthy_living_tribe_additional_template_locations( string $file, string $template ) {
	$new_locations = array();

	if ( ! tribe_events_views_v2_is_enabled() ) {
		$new_locations['themes_root'] = trailingslashit( get_theme_root() ) . 'healthy-living/tribe-events';
	}
...
add_filter( 'tribe_events_template', 'healthy_living_tribe_additional_template_locations', 10, 2 );


/* Replace Styles */
function healthy_living_tribe_events_stylesheet_url() {
	if ( ! tribe_events_views_v2_is_enabled() ) {
...
add_action('wp_enqueue_scripts', 'healthy_living_tribe_events_stylesheet_url', 100);


/* Replace Pro Styles */
function healthy_living_tribe_events_pro_stylesheet_url() {
	if ( ! tribe_events_views_v2_is_enabled() ) {
...
add_action('wp_enqueue_scripts', 'healthy_living_tribe_events_pro_stylesheet_url', 100);


/* Replace Widget Styles */
function healthy_living_tribe_events_pro_widget_calendar_stylesheet_url() {
	if ( ! tribe_events_views_v2_is_enabled() ) {
		return '';
	}
}

add_filter('tribe_events_pro_widget_calendar_stylesheet_url', 'healthy_living_tribe_events_pro_widget_calendar_stylesheet_url');


/* Replace Responsive Styles */
function healthy_living_tribe_events_mobile_breakpoint() {
	if ( ! tribe_events_views_v2_is_enabled() ) {
		return 768;
	}
}

add_filter('tribe_events_mobile_breakpoint', 'healthy_living_tribe_events_mobile_breakpoint');


/* Events Archive, Venue and Organizer Layout */
function healthy_living_tribe_events_layout($cmsmasters_layout) {
	if (
...
add_filter('cmsmasters_theme_page_layout_filter', 'healthy_living_tribe_events_layout');
