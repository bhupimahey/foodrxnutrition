...
$excerpt = (in_array('excerpt', $cmsmasters_project_metadata) && healthy_living_project_check_exc_cont()) ? true : false;
...
<article id="post-<?php the_ID(); ?>" <?php post_class('cmsmasters_project_grid'); echo (($project_categs != '') ? ' data-category="' . esc_attr($project_categs) . '"' : '') ?>>
	...
	<?php 
		healthy_living_thumb_rollover(get_the_ID(), $project_thumb_size, false, $rollover, false, false, false, false, false, $project_thumb_high, true, $cmsmasters_project_link_redirect, $cmsmasters_project_link_url, $cmsmasters_project_link_target);
		
		
		if ($title || $categories || $excerpt || $likes || $comments) {
			echo '<div class="project_inner">';
				
				$title ? healthy_living_project_heading(get_the_ID(), 'h4', $cmsmasters_project_link_redirect, $cmsmasters_project_link_url, $cmsmasters_project_link_target) : '';
					...
					echo '<div class="cmsmasters_project_meta_info">';
						...
						if ($categories) {
							echo '<div class="cmsmasters_project_cont_info entry-meta">';
								...
								healthy_living_get_project_category(get_the_ID(), 'pj-categs', 'page', 'grid');
						...
						if ($likes || $comments) {
							echo '<footer class="cmsmasters_project_footer entry-meta">';
								...
								$likes ? healthy_living_get_project_likes('page') : '';
								
								$comments ? healthy_living_get_project_comments('page') : '';
				...
				$excerpt ? healthy_living_project_exc_cont() : '';
		...
		if (!$title) {
			echo '<div class="dn">';
				healthy_living_project_heading(get_the_ID(), 'h6');
			echo '</div>';
		}
