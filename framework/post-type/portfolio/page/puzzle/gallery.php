...
<article id="post-<?php the_ID(); ?>" <?php post_class('cmsmasters_project_puzzle ' . $cmsmasters_project_size); echo (($project_categs != '') ? ' data-category="' . esc_attr($project_categs) . '"' : '') ?>>
	...
	<?php 
		healthy_living_thumb_rollover(get_the_ID(), 'full', false, $rollover, false, false, false, false, false, false, true, $cmsmasters_project_link_redirect, $cmsmasters_project_link_url, $cmsmasters_project_link_target);
				...
				'<div class="project_inner_cont">';
					...
					if ($categories) {
						echo '<div class="cmsmasters_project_cont_info entry-meta">';
							...
							healthy_living_get_project_category(get_the_ID(), 'pj-categs', 'page', 'puzzle');
