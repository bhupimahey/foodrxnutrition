...
healthy_living_locate_template(CMSMASTERS_OPTIONS . '/cmsmasters-theme-options-general.php', true);
healthy_living_locate_template(CMSMASTERS_OPTIONS . '/cmsmasters-theme-options-post.php', true);
healthy_living_locate_template(CMSMASTERS_OPTIONS . '/cmsmasters-theme-options-page.php', true);
healthy_living_locate_template(CMSMASTERS_OPTIONS . '/cmsmasters-theme-options-other.php', true);


if (class_exists('Cmsmasters_Projects')) {
	healthy_living_locate_template(CMSMASTERS_OPTIONS . '/cmsmasters-theme-options-project.php', true);
}


if (class_exists('Cmsmasters_Profiles')) {
	healthy_living_locate_template(CMSMASTERS_OPTIONS . '/cmsmasters-theme-options-profile.php', true);
}

if (CMSMASTERS_TIMETABLE) {
	healthy_living_locate_template(CMSMASTERS_OPTIONS . '/cmsmasters-theme-options-tt-event.php', true);
}

if (CMSMASTERS_DONATIONS) {
	healthy_living_locate_template('cmsmasters-donations/admin/cmsmasters-theme-options-campaign.php', true);
	healthy_living_locate_template('cmsmasters-donations/admin/cmsmasters-theme-options-donation.php', true);
}


healthy_living_locate_template(CMSMASTERS_OPTIONS . '/cmsmasters-theme-options-product.php', true);


if (!function_exists('healthy_living_get_custom_all_meta_fields')) {
function healthy_living_get_custom_all_meta_fields() {
	$custom_meta_fields = healthy_living_get_custom_general_meta_fields();
	...
	if ( 
		...
		$custom_page_meta_fields = healthy_living_get_custom_page_meta_fields();
		...
		$custom_project_meta_fields = healthy_living_get_custom_project_meta_fields();
		...
		$custom_profile_meta_fields = healthy_living_get_custom_profile_meta_fields();
		...
		$custom_product_meta_fields = healthy_living_get_custom_product_meta_fields();
		...
		$custom_campaign_meta_fields = healthy_living_get_custom_campaign_meta_fields();
		...
		$custom_donation_meta_fields = healthy_living_get_custom_donation_meta_fields();
		...
		$custom_post_meta_fields = healthy_living_get_custom_post_meta_fields();
		
		$custom_all_meta_fields = array_merge($custom_post_meta_fields, $custom_meta_fields);
	} else {
		$custom_other_meta_fields = healthy_living_get_custom_other_meta_fields();
...
function healthy_living_admin_enqueue_scripts($hook) {
	if ( 
		($hook == 'post.php') || 
		($hook == 'post-new.php') 
	) {
		...
		wp_localize_script('cmsmasters_theme_options_js', 'cmsmasters_options', array( 
			'create_gallery' => 	esc_attr__('Create Gallery', 'healthy-living'), 
			'select_format' => 		esc_attr__('Please select the format.', 'healthy-living'), 
			'link_exists' => 		esc_html__('Link with this format already exists.', 'healthy-living'), 
			'want_remove' => 		esc_html__('Do you really want to remove this item?', 'healthy-living'), 
...
add_action('admin_enqueue_scripts', 'healthy_living_admin_enqueue_scripts');


function healthy_living_show_meta_box() {
	global $post;
	
	$custom_all_meta_fields = healthy_living_get_custom_all_meta_fields();
	...
	foreach ($custom_all_meta_fields as $field) {
		...
		case 'select_sidebar':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Default Sidebar', 'healthy-living') . '</option>';
		...
		case 'select_slider':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Select Slider', 'healthy-living') . '</option>';
		...
		case 'select_post_categ':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Select Blog Category', 'healthy-living') . '</option>';
		...
		case 'select_project_cat':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Select Project Category', 'healthy-living') . '</option>';
		...
		case 'select_pl_categ':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Select Profile Category', 'healthy-living') . '</option>';
		...
		case 'image':
			...
			echo '<div class="cmsmasters_upload_parent cmsmasters_select_parent">' . 
				'<input type="button" id="cmsmasters_upload_' . $field['id'] . '_button" class="cmsmasters_upload_button button button-large" value="' . esc_attr__('Choose Image', 'healthy-living') . '" data-title="' . esc_attr__('Choose Image', 'healthy-living') . '" data-button="' . esc_attr__('Insert Image', 'healthy-living') . '" data-id="cmsmasters-media-select-frame-' . $field['id'] . '" data-classes="media-frame cmsmasters-media-select-frame' . ((!isset($field['description'])) ? ' cmsmasters-frame-no-description' : '') . ((!isset($field['caption'])) ? ' cmsmasters-frame-no-caption' : '') . ((!isset($field['align'])) ? ' cmsmasters-frame-no-align' : '') . ((!isset($field['link'])) ? ' cmsmasters-frame-no-link' : '') . ((!isset($field['size'])) ? ' cmsmasters-frame-no-size' : '') . '" data-library="image" data-type="' . $field['frame'] . '"' . (($field['frame'] === 'post') ? ' data-state="insert"' : '') . ' data-multiple="' . $field['multiple'] . '" />' . 
		...
		case 'icon':
			echo '<div class="icon_management">' . 
				'<p>' . 
					...
					'<input id="' . $field['id'] . '_button" class="cmsmasters_icon_choose_button button" type="button" value="' . esc_attr__('Choose icon', 'healthy-living') . '" />' . 
					'<a href="#" class="cmsmasters_remove_icon admin-icon-remove" title="' . esc_attr__('Remove icon', 'healthy-living') . '"' . (($meta != '') ? ' style="display:inline-block;"' : '') . '></a>' . 
				'</p>' . 
			'</div>' . 
		...
		case 'repeatable_link':
			...
			echo '<div class="ovh">' . 
				'<div class="fl"><strong>' . esc_html__('Title', 'healthy-living') . '</strong></div>' . 
				'<div class="fl"><strong>' . esc_html__('Link', 'healthy-living') . '</strong></div>' . 
			'</div>' . 
			...
			'<select name="' . $field['id'] . '-select" id="' . $field['id'] . '-select">' . 
				'<optgroup label="' . esc_attr__('Blank Field', 'healthy-living') . '">' . 
					'<option value="">' . esc_html__('Select Link', 'healthy-living') . '</option>' . 
				'</optgroup>' . 
				'<optgroup label="' . esc_attr__('Posts', 'healthy-living') . '">';
			...
			echo '</optgroup>' . 
				'<optgroup label="' . esc_attr__('Pages', 'healthy-living') . '">';
			...
			echo '</optgroup>' . 
				'<optgroup label="' . esc_attr__('Projects', 'healthy-living') . '">';
		...
		case 'repeatable_multiple':
			echo '<div class="ovh">' . 
				'<div class="fl"><strong>' . esc_html__('Title', 'healthy-living') . '</strong></div>' . 
				'<div class="fl"><strong>' . esc_html__('Values', 'healthy-living') . '</strong></div>' . 
			'</div>' . 
			'<ul id="' . $field['id'] . '-repeatable" class="custom_repeatable">';
		...
		case 'repeatable_media':
			echo '<select name="' . $field['id'] . '-select" id="' . $field['id'] . '-select">' . 
				'<option value="">' . esc_html__('Select Format', 'healthy-living') . ' &nbsp;</option>';
		...
		case 'images_list':
			...
			echo '<div class="cmsmasters_upload_parent cmsmasters_gallery_parent">' . 
				'<input type="button" id="cmsmasters_gallery_' . $field['id'] . '_button" class="cmsmasters_upload_button button button-large" value="' . (($meta != '') ? esc_attr__('Edit Gallery', 'healthy-living') : esc_attr__('Create Gallery', 'healthy-living')) . '" data-title="' . esc_attr__('Create/Edit Gallery', 'healthy-living') . '" data-button="' . esc_attr__('Insert Gallery', 'healthy-living') . '" data-id="cmsmasters-media-select-frame-' . $field['id'] . '" data-classes="media-frame cmsmasters-media-gallery-frame' . ((!isset($field['description'])) ? ' cmsmasters-frame-no-description' : '') . ((!isset($field['caption'])) ? ' cmsmasters-frame-no-caption' : '') . ((!isset($field['align'])) ? ' cmsmasters-frame-no-align' : '') . ((!isset($field['link'])) ? ' cmsmasters-frame-no-link' : '') . ((!isset($field['size'])) ? ' cmsmasters-frame-no-size' : '') . '" data-library="image" data-type="' . $field['frame'] . '"' . (($field['frame'] == 'post') ? ' data-state="' . (($meta != '') ? 'gallery-edit' : 'gallery-library') . '"' : '') . ' data-multiple="' . $field['multiple'] . '"' . (($meta != '') ? ' data-editing="true"' : '') . ' />' . 
				...
				foreach ($ids as $id) {
					if (isset($id[0]) && isset($id[1])) {
						echo '<li class="cmsmasters_gallery_item">' . 
							...
							'<a href="#" class="cmsmasters_gallery_cancel admin-icon-remove" title="' . esc_attr__('Remove', 'healthy-living') . '"></a>' . 
						'</li>';
					}
				}
			}
		...
		case 'funds':
			...
			echo '<div class="cmsmasters_funds_stats">' . 
				'<div class="cmsmasters_funds_stat_wrap">' . 
					...
					'</div>' . 
				'</div>' . 
				'<span class="fl">' . esc_html__('count', 'healthy-living') . ' - <strong>' . $funds_number . '</strong></span>' . 
				'<span class="fr">' . cmsmasters_donations_currency(cmsmasters_number_format($target - $funds)) . ' ' . esc_html__('to go', 'healthy-living') . '</span>';
			'</div>' . 
			(($field['desc'] != '') ? '<br />' . '<span class="description">' . $field['desc'] . '</span>': '');
...
function healthy_living_save_custom_meta($post_id) {
    $custom_all_meta_fields = healthy_living_get_custom_all_meta_fields();
...
add_action('save_post', 'healthy_living_save_custom_meta');


function healthy_living_add_custom_meta_box() {
	$args = array(
	   'public' => true 
	);
	...
	foreach ($screens as $screen) {	
		add_meta_box( 
			...
			esc_html__('Theme Options', 'healthy-living'), 
			'healthy_living_show_meta_box', 
			$screen, 
			'normal', 
			'high' 
		);
...
add_action('add_meta_boxes', 'healthy_living_add_custom_meta_box');
