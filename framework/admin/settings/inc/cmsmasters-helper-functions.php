...
function healthy_living_get_admin_page() {
	global $pagenow;
...
function healthy_living_default_tab() {
	$current_page = healthy_living_get_admin_page();
...
function healthy_living_get_the_tab() {
	global $pagenow;
	
	$default_tab = healthy_living_default_tab();
...
function healthy_living_settings_page_header() {
	...
	if ($tabs != '') {
		...
		if (class_exists('Cmsmasters_Custom_Fonts') && $page == 'cmsmasters-settings-font') {
			$links[] = '<a class="' . $class . '" href="edit.php?post_type=cmsmasters_font">' . esc_html__('Custom Fonts', 'healthy-living') . '</a>';
		}
