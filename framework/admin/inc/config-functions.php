...
if (!function_exists('cmsmasters_fonts_list')) {
	function cmsmasters_fonts_list() {
		...
		if (isset($cmsmasters_option['healthy-living' . '_google_web_fonts']) && is_array($cmsmasters_option['healthy-living' . '_google_web_fonts'])) {
			$google_web_fonts_array = $cmsmasters_option['healthy-living' . '_google_web_fonts'];
		...
		$fonts = array( 
			'' => esc_html__('None', 'healthy-living'), 
			'local' => $local_fonts, 
			'web' => $google_web_fonts 
		);
		
		
		return apply_filters('healthy_living_google_fonts_list_filter', $fonts);
	}
}


// Add google web fonts if empty
function healthy_living_add_google_web_fonts_first() {
	$font_google = get_option( 'cmsmasters_options_' . 'healthy-living' . '_font_google', array() );
	...
	$fonts = array(
		'healthy-living' . '_google_web_fonts' => cmsmasters_google_fonts_list(),
		'healthy-living' . '_google_web_fonts_subset' => '',
	);

	update_option( 'cmsmasters_options_' . 'healthy-living' . '_font_google', $fonts );
}

add_action('init', 'healthy_living_add_google_web_fonts_first');


// Theme Settings Google Fonts List
if (!function_exists('cmsmasters_google_fonts_list')) {
	function cmsmasters_google_fonts_list() {
		$fonts = healthy_living_get_google_fonts_list();
...
if (!function_exists('cmsmasters_image_thumbnail_list')) {
	function cmsmasters_image_thumbnail_list() {
		$list = healthy_living_get_image_thumbnail_list();
...
if (!function_exists('cmsmasters_color_schemes_list')) {
	function cmsmasters_color_schemes_list() {
		$list = healthy_living_all_color_schemes_list();
