...
function healthy_living_post_heading($cmsmasters_id, $tag = 'h1', $show = true) { 
	$out = '';
...
function healthy_living_post_title_nolink($cmsmasters_id, $tag = 'h1', $show = true) { 
	$out = '';
...
function healthy_living_get_post_date($template_type = 'page', $layout_type = 'default', $show = true) {
...
function healthy_living_get_post_author($template_type = 'page', $show = true) {
	if ($template_type == 'page') {
		$out = '<span class="cmsmasters_post_author">' . 
			esc_html__('by', 'healthy-living') . ' ' . 
			'<a href="' . esc_url(get_author_posts_url(get_the_author_meta('ID'))) . '" title="' . esc_attr__('Posts by', 'healthy-living') . ' ' . esc_attr(get_the_author_meta('display_name')) . '" class="vcard author">' . 
		...
		if ($cmsmasters_option['healthy-living' . '_blog_post_author']) {
			$out .= '<span class="cmsmasters_post_author">' . 
				esc_html__('by', 'healthy-living') . ' ' . 
				'<a href="' . esc_url(get_author_posts_url(get_the_author_meta('ID'))) . '" title="' . esc_attr__('Posts by', 'healthy-living') . ' ' . esc_attr(get_the_author_meta('display_name')) . '" class="vcard author">' . 
					'<span class="fn">' . esc_html(get_the_author_meta('display_name')) . '</span>' . 
...
function healthy_living_get_post_category($cmsmasters_id, $taxonomy, $template_type = 'page', $show = true) {
	$out = '';
	...
	if (get_the_terms($cmsmasters_id, $taxonomy)) {
		if ($template_type == 'page') {
			...
			if ($cmsmasters_option['healthy-living' . '_blog_post_cat']) {
				$out .= '<span class="cmsmasters_post_category">' . 
					esc_html__('in', 'healthy-living') . ' ' . 
					healthy_living_get_the_category_list($cmsmasters_id, $taxonomy, ', ') . 
				'</span>';
...
function healthy_living_get_post_tags($show = true) {
	if (get_the_tags()) {
		$cmsmasters_option = healthy_living_get_global_options();
		$out = '';
		
		if ($cmsmasters_option['healthy-living' . '_blog_post_tag']) {
			$out .= '<span class="cmsmasters_post_tags">' . 
				get_the_tag_list(esc_html__('tags', 'healthy-living') . ' ', ', ', '') . 
			'</span>';
		}
...
function healthy_living_post_exc_cont($template_type = 'page', $layout_type = 'default', $show = true) {
	if ($template_type == 'page') {
		if ($layout_type == 'default') {
			$out = cmsmasters_divpdel('<div class="cmsmasters_post_content entry-content">' . "\n" . 
				wpautop(theme_excerpt(60, false)) . 
			'</div>' . "\n");
		} elseif (($layout_type == 'masonry') || ($layout_type == 'timeline')) {
			$out = cmsmasters_divpdel('<div class="cmsmasters_post_content entry-content">' . "\n" . 
				wpautop(theme_excerpt(22, false)) . 
			'</div>' . "\n");
		}
	} elseif ($template_type == 'post') {
		$out = cmsmasters_divpdel('<div class="cmsmasters_post_content entry-content">' . "\n" . 
...
function healthy_living_get_post_likes($template_type = 'page', $show = true) {
	$out = '';
	...
	if ($template_type == 'page') {
		...
		if ($cmsmasters_option['healthy-living' . '_blog_post_like']) {
			$out = cmsmastersLike('cmsmasters_post_likes');
		}
	}
...
function healthy_living_get_post_comments($template_type = 'page', $show = true) {
	$out = '';
	
	
	if (comments_open()) {
		if ($template_type == 'page') {
			$out = healthy_living_get_comments('cmsmasters_post_comments');
		} elseif ($template_type == 'post') {
			$cmsmasters_option = healthy_living_get_global_options();
			
			if ($cmsmasters_option['healthy-living' . '_blog_post_comment']) {
				$out = healthy_living_get_comments('cmsmasters_post_comments');
			}
		}
	}
...
function healthy_living_post_more($cmsmasters_id, $show = true) {
	$cmsmasters_post_read_more = get_post_meta($cmsmasters_id, 'cmsmasters_post_read_more', true);
	
	
	if ($cmsmasters_post_read_more == '') {
		$cmsmasters_post_read_more = esc_html__('Continue Reading...', 'healthy-living');
	}
...
function healthy_living_post_format_icon_placeholder($cmsmasters_id, $format = 'std', $show = true) {
	$out = '<figure>' . 
