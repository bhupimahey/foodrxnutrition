...
function healthy_living_profile_heading($cmsmasters_id, $layout = 'horizontal', $tag = 'h1', $sub_title = false, $tag_sub = 'h3', $show = true) { 
...
function healthy_living_profile_title_nolink($cmsmasters_id, $tag = 'h1', $sub_title = false, $tag_sub = 'h5', $show = true) { 
	$out = '<' . esc_html($tag) . ' class="cmsmasters_profile_title entry-title">' . 
...
function healthy_living_profile_exc_cont($layout = 'horizontal', $show = true) {
	if ($layout == 'horizontal') {
		...
		$out = cmsmasters_divpdel('<div class="cmsmasters_profile_content entry-content">' . "\n" . 
			wpautop(theme_excerpt(65, false)) . 
		'</div>' . "\n");
	}
...
function healthy_living_get_profile_category($cmsmasters_id, $taxonomy, $template_type = 'page', $show = true) {
	$out = '';
	...
	if (get_the_terms($cmsmasters_id, $taxonomy)) {
		if ($template_type == 'page') {
			...
			if ($cmsmasters_option['healthy-living' . '_profile_post_cat']) {
				$out .= '<div class="profile_details_item">' . 
					'<div class="profile_details_item_title">' . esc_html__('Categories', 'healthy-living') . ':' . '</div>' . 
...
function healthy_living_get_profile_likes($template_type = 'page', $show = true) {
	$out = '';
	...
	if ($template_type == 'page') {
		...
		if ($cmsmasters_option['healthy-living' . '_profile_post_like']) {
			$out = '<div class="profile_details_item">' . 
				'<div class="profile_details_item_title">' . esc_html__('Likes', 'healthy-living') . ':' . '</div>' . 
...
function healthy_living_get_profile_comments($template_type = 'page', $show = true) {
	$out = '';
	
	
	if (comments_open()) {
		if ($template_type == 'page') {
			$out = healthy_living_get_comments('cmsmasters_profile_comments');
		} elseif ($template_type == 'post') {
			$cmsmasters_option = healthy_living_get_global_options();
			
			if ($cmsmasters_option['healthy-living' . '_profile_post_comment']) {
				$out .= '<div class="profile_details_item">' . 
					'<div class="profile_details_item_title">' . esc_html__('Comments', 'healthy-living') . ':' . '</div>' . 
					'<div class="profile_details_item_desc details_item_desc_comments">' . 
						healthy_living_get_comments('cmsmasters_profile_comments') . 
					'</div>' . 
				'</div>';
			}
		}
...
function healthy_living_get_profile_features($features_position = 'features', $features = '', $features_title = false, $tag = 'h2', $show = true) {
	if (
		!empty($features[0][0]) && 
		!empty($features[0][1])
	) {
...
function healthy_living_profile_social_icons($social_icons, $title_box = false, $tag = 'h2', $show = true) {
	if ($social_icons != '') {
